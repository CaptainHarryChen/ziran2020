...
template <class T, int dim> class CouplingInitBase;

template <class T> class CouplingInit3D : public CouplingInitBase<T, 3> {
public:
...
#define SET_MPM_VALUE(A, B) sim1.A = sim2.A = B
  ...
  void set_simulator(T target_dx, T dx, T dy, T dz, T h = -1,
                     bool bb_box = true) {
    if (h < 0) {
      h = 1 - dy - dy;
    }
    ...
    sim_coordinator.setSolver(1e-3, -1, 10000,
                              CouplingSimulation<T, dim>::AMGCL);
    ...
    sim_coordinator.e2.implicit_penalty_collision =
        (T)1000 * std::pow((T)sim_coordinator.e2.dx, (T)dim) / (T)9 * 1000000;
    ...
    if (bb_box) {
      ...
      HalfSpace<T, dim> ground_ls(
          TV(1 + dx - half_dx, 1 + dy - half_dx, 1 + dz - half_dx),
          TV(0, 1, 0));
      AnalyticCollisionObject<T, dim> ground_object(
          ground_ls, AnalyticCollisionObject<T, dim>::SLIP);
      sim1.sampleBoundaryParticlesInCollisionObjects(
          ground_object,
          TV(1 + dx - three_dx, 1 + dy - three_dx, 1 + dz - three_dx),
          TV(2 - dx + three_dx, 1 + dy + three_dx, 2 - dz + three_dx),
          sim1.dx * 2);
      init_helper1.addAnalyticCollisionObject(ground_object);
      HalfSpace<T, dim> left_wall_ls(
          TV(1 + dx - half_dx, 1 + dy - half_dx, 1 + dz - half_dx),
          TV(1, 0, 0));
      AnalyticCollisionObject<T, dim> left_wall_object(
          left_wall_ls, AnalyticCollisionObject<T, dim>::SLIP);
      sim1.sampleBoundaryParticlesInCollisionObjects(
          left_wall_object,
          TV(1 + dx - three_dx, 1 + dy - three_dx, 1 + dz - three_dx),
          TV(1 + dx + three_dx, 1 + dy + h, 2 - dz + three_dx), sim1.dx * 2);
      init_helper1.addAnalyticCollisionObject(left_wall_object);
      HalfSpace<T, dim> back_wall_ls(
          TV(1 + dx - half_dx, 1 + dy - half_dx, 1 + dz - half_dx),
          TV(0, 0, 1));
      AnalyticCollisionObject<T, dim> back_wall_object(
          back_wall_ls, AnalyticCollisionObject<T, dim>::SLIP);
      sim1.sampleBoundaryParticlesInCollisionObjects(
          back_wall_object,
          TV(1 + dx - three_dx, 1 + dy - three_dx, 1 + dz - three_dx),
          TV(2 - dx + three_dx, 1 + dy + h, 1 + dz + three_dx), sim1.dx * 2);
      init_helper1.addAnalyticCollisionObject(back_wall_object);
      HalfSpace<T, dim> right_wall_ls(
          TV(2 - dx + half_dx, 2 - dy + half_dx, 2 - dz + half_dx),
          TV(-1, 0, 0));
      AnalyticCollisionObject<T, dim> right_wall_object(
          right_wall_ls, AnalyticCollisionObject<T, dim>::SLIP);
      sim1.sampleBoundaryParticlesInCollisionObjects(
          right_wall_object,
          TV(2 - dx - three_dx, 1 + dy - three_dx, 1 + dz - three_dx),
          TV(2 - dx + three_dx, 1 + dy + h, 2 - dz + three_dx), sim1.dx * 2);
      init_helper1.addAnalyticCollisionObject(right_wall_object);
      HalfSpace<T, dim> front_wall_ls(
          TV(2 - dx + half_dx, 2 - dy + half_dx, 2 - dz + half_dx),
          TV(0, 0, -1));
      AnalyticCollisionObject<T, dim> front_wall_object(
          front_wall_ls, AnalyticCollisionObject<T, dim>::SLIP);
      sim1.sampleBoundaryParticlesInCollisionObjects(
          front_wall_object,
          TV(1 + dx - three_dx, 1 + dy - three_dx, 2 - dz - three_dx),
          TV(2 - dx + three_dx, 1 + dy + h, 2 - dz + three_dx), sim1.dx * 2);
  ...
  void reload() override {
    if (test_number == 1) {
      ...
      ///////////////// SIM1 (FLUID)
      {
        ...
        MpmParticleHandleBase<T, dim> p_handle1 =
            init_helper1.sampleInAnalyticLevelSet(source_ls1, fluid_density,
                                                  ppc);
        LinearCorotated<T, dim> model1(E, nu);
        QuadraticVolumePenalty<T, dim> model2(E, nu);
        p_handle1.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_FLUID,
        ...
        sim_coordinator.end_time_step_callbacks.push_back(
            [this, source_id, fluid_density, ppc, model1, model2](int frame,
                                                                  int substep) {
              if (frame < 20) {
                // add more particles from source Collision object
                int N = init_helper1.sourceSampleAndPrune(source_id,
                                                          fluid_density, ppc);
                if (N) {
                  MpmParticleHandleBase<T, dim> source_particles_handle =
                      init_helper1.getParticlesFromSource(source_id,
                                                          fluid_density, ppc);
                  source_particles_handle.addFJMixedMpmForce(
                      model1, model2, MATERIAL_PHASE_FLUID,
                      MATERIAL_PROPERTY_INCOMPRESSIBLE,
                      /*linear corotated*/ false);
                }
              }
            });
      }
      ///////////////// SIM2
      {
        ...
        MpmParticleHandleBase<T, dim> p_handle22 =
            init_helper2.sampleFromVdbFileWithExistingPoints(
                meshed_points, "LevelSets/bear.vdb", 1000, 27);
        LinearCorotated<T, dim> model1(E, nu);
        QuadraticVolumePenalty<T, dim> model2(E, nu);
        p_handle22.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_SOLID,
                                      MATERIAL_PROPERTY_COMPRESSIBLE,
                                      /*linear corotated*/ true);
        SnowPlasticity<T> p(0, 0.9, 5); // restrict F to [1-0.9, 1+5]
        p_handle22.addPlasticity(model1, p, "F");
        Sphere<T, dim> dummy(TV(0, 0, 0), 100);
        p_handle22.parallelPinParticlesInLevelSet(
            dummy, 0, 0, /*collider spring mode*/ true);
      }
    }
    ...
    if (test_number == 2) {
      ...
      ///////////////// SIM1 (FLUID)
      {
        ...
        MpmParticleHandleBase<T, dim> p_handle1 =
            init_helper1.sampleInAnalyticLevelSet(
                sphere, /* fluid_density */ 1000,
                /* particles per dimension */ 27);
        LinearCorotated<T, dim> model1(E, nu);
        QuadraticVolumePenalty<T, dim> model2(E, nu);
        p_handle1.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_FLUID,
                                     MATERIAL_PROPERTY_INCOMPRESSIBLE,
        ...
        sim_coordinator.end_frame_callbacks.push_back([this, model1,
                                                       model2](int frame) {
          if (frame == 20) {
      ...
      ///////////////// SIM2
      {
        ...
        MpmParticleHandleBase<T, dim> p_handle22 =
            init_helper2.sampleFromVdbFileWithExistingPoints(
                meshed_points, "LevelSets/ball.vdb", 800, 27);
        p_handle22.transform([&](int index, Ref<T> mass, TV &X, TV &V) {
          X += TV(6.5, 1.5, 6.5);
        });
        p_handle22.addFJMixedMpmForce(
            model_corotated, model2, MATERIAL_PHASE_SOLID,
            MATERIAL_PROPERTY_COMPRESSIBLE, /*linear corotated*/ false);
        p_handle22.parallelPinParticlesInLevelSet(
            dummy, 0, 0, /*collider spring mode*/ true);
            ...
            [this, model_neo, model_stvk, model_linear, model2](int frame) {
              ...
              if (frame == 20) {
                ...
                std::string absolute_path =
                    DataDir().absolutePath("TriMesh/ball.obj");
                readPositionObj(absolute_path, meshed_points);
                MpmParticleHandleBase<T, dim> p_handle22 =
                    init_helper2.sampleFromVdbFileWithExistingPoints(
                        meshed_points, "LevelSets/ball.vdb", 800, 27);
                ...
                p_handle22.addFJMixedMpmForce(
                    model_neo, model2, MATERIAL_PHASE_SOLID,
                    MATERIAL_PROPERTY_COMPRESSIBLE, /*linear corotated*/ false);
                p_handle22.parallelPinParticlesInLevelSet(
                    dummy, 0, 0, /*collider spring mode*/ true);
              }
              ...
              if (frame == 40) {
                ...
                p_handle22.addFJMixedMpmForce(
                    model_stvk, model2, MATERIAL_PHASE_SOLID,
                    MATERIAL_PROPERTY_COMPRESSIBLE, /*linear corotated*/ false);
                p_handle22.parallelPinParticlesInLevelSet(
                    dummy, 0, 0, /*collider spring mode*/ true);
              }
    ...
    if (test_number == 3) {
      ...
      ///////////////// SIM1 (FLUID)
      {
        ...
        MpmParticleHandleBase<T, dim> p_handle21 =
            init_helper1.sampleInAnalyticLevelSet(ball, 1000, ppc);
        p_handle21.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_FLUID,
                                      MATERIAL_PROPERTY_INCOMPRESSIBLE,
                                      /*linear corotated*/ true);
        ...
        sim_coordinator.end_time_step_callbacks.push_back(
            [this, source_id, fluid_density, ppc, model1, model2](int frame,
              ...
              if (frame < 130 && frame > 70) {
                ...
                if (N) {
                  MpmParticleHandleBase<T, dim> source_particles_handle =
                      init_helper1.getParticlesFromSource(source_id, 1500, ppc);
                  source_particles_handle.addFJMixedMpmForce(
                      model1, model2, MATERIAL_PHASE_FLUID,
                      MATERIAL_PROPERTY_INCOMPRESSIBLE,
                      /*linear corotated*/ true);
      ...
      ///////////////// SIM2
      {
        ...
        MpmParticleHandleBase<T, dim> p_handle22 =
            init_helper2.sampleFromVdbFileWithExistingPoints(
                meshed_points, "LevelSets/sheet.vdb", solid_density, 27);
        p_handle22.addFJMixedMpmForce(
            model_linear, model2, MATERIAL_PHASE_SOLID,
            MATERIAL_PROPERTY_COMPRESSIBLE, /*linear corotated*/ true);
        p_handle22.parallelPinParticlesInLevelSet(
            dummy, 0, 0, /*collider spring mode*/ true);
        SnowPlasticity<T> p(0, 0.9, 5); // restrict F to [1-0.9, 1+5]
        p_handle22.addPlasticity(model_linear, p, "F");
        sim_coordinator.end_time_step_callbacks.emplace_back(
            [&particles = sim_coordinator.e2.particles,
             &dt = sim_coordinator.e2.dt, &dx = sim_coordinator.e2.dx,
             solid_density](int frame, int substep) {
              if (frame < 40) {
                auto *phase_pointer =
                    particles.DataManager::getPointer(material_phase_name());
                for (int i = 0; i < particles.count; i++)
                  if ((*phase_pointer)[i] == MATERIAL_PHASE_SOLID) {
                    T coeff = 0.3;
                    T area = dx * dx * M_PI;
                    TV resistant_f = -0.5 * solid_density *
                                     particles.V(i).norm() *
                                     particles.V(i).norm() * coeff * area *
                                     particles.V(i).normalized();
    ...
    if (test_number == 4) {
      ...
      sim_coordinator.e2.newton.linesearch = true;
      sim_coordinator.e2.objective.switchLinearSolver(
          ZIRAN::SimulationBase::LinearSolverType(1));
      sim_coordinator.e2.objective.linear_solver->max_iterations = 10000;
      sim_coordinator.e2.objective.linear_solver->tolerance = 1e-6;
      sim_coordinator.e2.implicit_penalty_collision =
          solid_density * std::pow((T)sim_coordinator.e2.dx, (T)dim) / (T)ppc *
          1000000;
      ...
      {
        {
          AxisAlignedAnalyticBox<T, dim> box(TV(0, 0, 0) + scene_shift,
                                             TV(2, 0.2, 1) + scene_shift);
          MpmParticleHandleBase<T, dim> p_handle1 =
              init_helper1.sampleInAnalyticLevelSet(box, fluid_density, ppc);
          p_handle1.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_FLUID,
                                       MATERIAL_PROPERTY_INCOMPRESSIBLE,
                                       /*linear corotated*/ false);
        }
        {
          AxisAlignedAnalyticBox<T, dim> box(TV(0, 0.2, 0.3) + scene_shift,
                                             TV(0.5, 1, 0.7) + scene_shift);
          MpmParticleHandleBase<T, dim> p_handle1 =
              init_helper1.sampleInAnalyticLevelSet(box, fluid_density, ppc);
          p_handle1.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_FLUID,
                                       MATERIAL_PROPERTY_INCOMPRESSIBLE,
        ...
        AxisAlignedAnalyticBox<T, dim> box1(
            TV(0, 0, 0) - 4 * sim_coordinator.e1.dx * TV::Ones() + scene_shift,
            TV(2, 4, 1) + 4 * sim_coordinator.e1.dx * TV::Ones() + scene_shift);
        AxisAlignedAnalyticBox<T, dim> box2(TV(0, 0, 0) + scene_shift,
                                            TV(2, 4, 1) + scene_shift);
        DifferenceLevelSet<T, dim> ls1, ls2;
        ls1.add(box1, box2);
        AnalyticCollisionObject<T, dim> container_object(
            ls1, AnalyticCollisionObject<T, dim>::SLIP);
        sim1.sampleBoundaryParticlesInCollisionObjects(
            container_object,
            TV(0, 0, 0) - 4 * sim_coordinator.e1.dx * TV::Ones() + scene_shift,
            TV(2, 4, 2) + 4 * sim_coordinator.e1.dx * TV::Ones() + scene_shift,
            sim1.dx * 2);
      ...
      {
        {
          ...
          MpmParticleHandleBase<T, dim> p_handle =
              init_helper2.sampleInAnalyticLevelSet(sphere, solid_density, ppc);
          p_handle.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_SOLID,
                                      MATERIAL_PROPERTY_COMPRESSIBLE,
                                      /*linear corotated*/ false);
          Sphere<T, dim> dummy(TV(0, 0, 0), 100);
          p_handle.parallelPinParticlesInLevelSet(
              dummy, 0, 0, /*collider spring mode*/ true);
        }
        AxisAlignedAnalyticBox<T, dim> box1(
            TV(0, 0, 0) - 4 * sim_coordinator.e1.dx * TV::Ones() + scene_shift,
            TV(2, 4, 1) + 4 * sim_coordinator.e1.dx * TV::Ones() + scene_shift);
        AxisAlignedAnalyticBox<T, dim> box2(
            TV(0, 0, 0) + sim2.dx * TV::Ones() + scene_shift,
            TV(2, 4, 1) - sim2.dx * TV::Ones() + scene_shift);
        DifferenceLevelSet<T, dim> ls1, ls2;
        ls1.add(box1, box2);
        AnalyticCollisionObject<T, dim> container_object(
            ls1, AnalyticCollisionObject<T, dim>::GHOST);
    ...
    if (test_number == 5) {
      ...
      ///////////////// SIM1
      {
        ...
        sim_coordinator.end_frame_callbacks.push_back(
            [this, scene_shift, fluid_density, ppc, model1, model2](int frame) {
              if (frame == 10) {
                TV source_center1(1 + scene_shift, 2 + scene_shift,
                                  1 + scene_shift);
                Sphere<T, dim> source_ls1(source_center1, 0.25);
                MpmParticleHandleBase<T, dim> p_handle1 =
                    init_helper1.sampleInAnalyticLevelSet(source_ls1,
                                                          fluid_density, ppc);
                p_handle1.addFJMixedMpmForce(model1, model2,
      ...
      ///////////////// SIM2
      {
        ...
        std::string absolute_path =
            DataDir().absolutePath("TetMesh/twohead500k.mesh");
        ...
        MeshHandle<T, dim, TMesh> mesh = scene2.createMesh(
            (std::function<void(TMesh &, StdVector<TV> &)>)reader);
        auto &x_array = mesh.particles.X.array;
        DeformableObjectHandle<T, dim, TMesh> deformable =
            scene2.addDeformableObject(mesh);
        deformable.setMassFromDensity(fluid_density);
        deformable.addFemHyperelasticForce(model1);
        auto position_shift_to_right = [&](int index, Ref<T> mass, TV &X,
                                           TV &V) { X += TV(1, 1, 1); };
        deformable.transform(position_shift_to_right);
        MpmParticleHandleBase<T, dim> p_handle22 =
            init_helper2.makeParticleHandle(deformable, (T)1);
        p_handle22.addFJMixedMpmForce(model1, model2, MATERIAL_PHASE_SOLID,
                                      MATERIAL_PROPERTY_COMPRESSIBLE,
                                      /*linear corotated*/ true);
        SnowPlasticity<T> p(0, 0.9, 5); // restrict F to [1-0.9, 1+5]
        ...
        AnalyticCollisionObject<T, dim> container_object(
            sphere_ls, AnalyticCollisionObject<T, dim>::GHOST);
